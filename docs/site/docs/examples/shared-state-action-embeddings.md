...
keeps one independent matrix and response vector per option. When you
add a 7th option to a 6-option capsule, the new option's matrix is
fresh — its UCB score is driven entirely by the exploration bonus
until it accumulates enough observations. For a capsule whose option
set rotates (LLM models, server flavours, retail variants), that's a
real cost: every rotation pays the cold-start tax.
...
estimates land within ~4% of the true expected reward in the
linear-truth case; the assertion threshold is 30% to leave RNG-seed
headroom.
...
posterior mean shift, because they share parameter mass with B. That
...
- If the option set is fixed and tiny (≤ 5 options), the cold-start
  saving doesn't outweigh the cost of authoring features. Stick with
  per-option LinUCB or Thompson sampling on Bernoulli rewards.
...
- [Strategy node](../concepts/strategy-node.md) — the three adaptive
  flavors.
- [Hierarchical region routing](hierarchical-region-routing.md) — the
  other non-default flavor.
